...
{
    public class VerificationEmailSender(IOptions<VerificationOptions> options) : IVerificationEmailSender
    {
        ...
        public async Task SendEmail(string email, string verificationLink)
        {
            ...
            using (var channel = GrpcChannel.ForAddress(options.Value.EmailServiceAddress))
            {
                ...
                var request = new SendMailRequest { Body = GenerateBody(verificationLink), Subject = SUBJECT };
